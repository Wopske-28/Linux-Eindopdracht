...
php -q add_device.php --description="$name" --ip="${addressArr[$count]}" --template=3 --avail=snmp --ping_method=tcp --ping_port="23" --ping_retries=2 --version=3 --port=161 --timeout=2000 --username="authPrivUser" --password="password" --authproto="MD5" --privpass="password" --privproto="DES"
((count++))
done
